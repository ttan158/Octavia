...
export default async function Page() {
  ...
  const categorisedSongs = songsWithUrls
    .filter(
  ...
  if (
    ...
    Object.keys(categorisedSongs).length === 0
  ) {
    return (
      <div className="flex h-full flex-col items-center justify-center p-4 text-center">
        <Music className="text-muted-foregroud h-20 w-20" />
        <h1 className="mt-4 text-2xl font-bold tracking-tight">
          No Music Here
        </h1>
        <p className="text-muted-foreground mt-2">
          There are no published songs available right now
        </p>
        <br />
  ...
  return (
    <div className="p-4">
      ...
      {Object.entries(categorisedSongs)
        .slice(0, 5)
        .map(([category, song]) => (
