...
import requests
from prompts import PROMPT_GENERATOR_PROMPT, LYRICS_GENERATOR_PROMPT, CATEGORY_GENERATION_PROMPT
import boto3
...
class MusicGenServer:
    ...
    def load_model(self):
        ...
        # Stable Diffusion Model (Image Gen) 
    ...
    def generate_categories(self, description: str) -> List[str]:
        full_prompt = CATEGORY_GENERATION_PROMPT.format(description=description)
        
        response = self.prompt_qwen(full_prompt)
        categories = [cat.strip() 
                      for cat in response.split(",") if cat.strip()]
        return categories

    def generate_and_uplaod_to_s3(
            self,
            prompt:str,
            lyrics: str,
            instrumental: bool,
            ...
            seed: int,
            description_for_categorisation: str
        ...
        # Category Generation
        categories = self.generate_categories(description_for_categorisation)
    ...
    def generate_from_description(self, request: GenerateFromDescriptionRequest) -> GenerateMusicResponseS3:
        ...
        return self.generate_and_uplaod_to_s3(prompt=prompt, lyrics=lyrics, description_for_categorisation=request.full_described_song, **request.model_dump(exclude={"full_described_song"}))

    
    @modal.fastapi_endpoint(method="POST", requires_proxy_auth=True)
    def generate_with_lyrics(self, request: GenerateWithCustomLyricsRequest) -> GenerateMusicResponseS3:
        return self.generate_and_uplaod_to_s3(prompt=request.prompt, lyrics=request.lyrics, description_for_categorisation=requests.prompt, **request.model_dump(exclude={"prompt", "lyrics"}))        
    ...
    def generate_with_described_lyrics(self, request: GenerateWithDescribedLyricsRequest) -> GenerateMusicResponseS3:
        ...
        return self.generate_and_uplaod_to_s3(prompt=request.prompt, lyrics=lyrics, description_for_categorisation=requests.request.prompt, **request.model_dump(exclude={"described_lyrics", "prompt"}))
...
def main():
    ...
    request_data = GenerateFromDescriptionRequest(
        full_described_song="A song about unemployment from a guy called Tony",
        guidance_scale=7.5
    )
